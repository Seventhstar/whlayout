...
(function() {
  ...
  this.disable_input = function(cancel) {
    ...
    return $cell.html('<span class="icon edit" item_id="' + item_id + '"></span><span class="icon delete" item_id="' + item_id + '"></span>');
  };
  ...
  $(document).ready(function() {
    $('.container').on('click', 'span.edit', function() {
      ...
      return $cell.html('<span class="icon icon_apply" item_id="' + item_id + '"></span><span class="icon icon_cancel" item_id="' + item_id + '"></span>');
    });
    $('.container').on('click', 'span.icon_cancel', function() {
      return disable_input();
    });
    return $('.container').on('click', 'span.icon_apply', function() {
      var inputs, item_id, model;
      model = $(this).closest('table').attr('model');
      item_id = $(this).attr('item_id');
      inputs = $('input[name^=upd]');
      return upd_param(inputs.serialize() + '&model=' + model + '&id=' + item_id);
    });
  });
...
(function() {
  ...
  this.update_goodsearch = function() {
    var param;
    param = $('form').serialize();
    return $.get('/goods_search?' + param, "", null, "script");
  };

  $(document).ready(function() {
    return $('#goods_search input').keyup(function() {
      var c, isWordcharacter;
...
(function() {
  this.update_layout_show = function(lay_id) {
    return $.get('/layouts/' + lay_id, "", null, "script");
  };

  $(document).ready(function() {
    $('.container').on('click', '#layout_elements span.btn-sm', function() {
      var lay_id, param;
      param = $('[name^=lay_el]');
    ...
    return $('.container').on('click', '#layout_elements span.delete', function() {
      var del, el_id, lay_id;
      lay_id = $('#lay_el_layout_id').val();
...
(function() {
  ...
  this.update_notesearch = function() {
    var param;
    param = $('form').serialize();
    return $.get('/note_search?' + param, "", null, "script");
  };

  $(document).ready(function() {
    $('#note_search input').keyup(function() {
      var c, isWordcharacter;
      c = String.fromCharCode(event.keyCode);
      isWordcharacter = c.match(/\w/);
      if (isWordcharacter || event.keyCode === 8) {
    ...
    return $('#search_category').chosen({
      width: '200px',
      disable_search: true
    }).on('change', function() {
      $(".goods_prices").spin();
      return update_notesearch();
    });
  });
...
(function() {
  $(document).ready(function() {
    return $(document).on('click', '.option_content span.delete', function() {
...
(function() {
  $(document).ready(function() {
    $('#search_url_search_category_id').chosen({
      width: '200px',
      disable_search: true
    });
    $('#search_url_search_site_id').chosen({
      width: '200px',
      disable_search: true
    });
    return $('#edit_search_url_1').on('click', '.copy', function() {
      return alert(copyFromClipboard());
    });
  });
...
(function() {
  ...
  this.update_whouse_show = function(wh_id) {
    return $.get('/whouses/' + wh_id, "", null, "script");
  };

  this.update_whel = function() {
    var param;
    param = $('form').serialize();
    $.get('whouse_elements/', param, null, 'script');
  };

  this.add_el_to_wh = function(dt, func) {
    $.ajax({
      url: '/ajax/add_wh_el',
      data: dt,
      type: 'POST',
  ...
  this.del_el_from_wh = function(el_id, func) {
    var del;
  ...
  $(document).ready(function() {
    $('#whouses_search select#whouse_id').chosen({
      width: '340px',
      disable_search: true
    }).on('change', function() {
      if (!$('#whouse_id').val()) {
    ...
    $('#whouses_search input').keyup(function() {
    ...
    $('.container').on('click', '#whouses_search span#btn-add-el', function() {
      var wh_id;
      wh_id = $('#whouse_id').val();
      return add_el_to_wh($('form').serialize(), 'update_whel()');
    });
    $('.container').on('click', '.whouse_elements span.btn-sm', function() {
      var param, wh_id;
      param = $('[name^=wh_el]');
      wh_id = $('#wh_el_whouse_id').val();
      return add_el_to_wh(param.serialize(), 'update_whouse_show(' + wh_id + ')');
    });
    $('.container').on('click', '.whouse_elements span.delete', function() {
      var el_id, wh_id;
      wh_id = $('#wh_el_whouse_id').val();
      el_id = $(this).attr('el_id');
